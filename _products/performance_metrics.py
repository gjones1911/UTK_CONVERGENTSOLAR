...
#                          TODO: my own loss functions
def g_ev():
    def explained_variance(y_true, y_pred):
        return -Rvar(y_true, y_pred)
    return explained_variance

def g_evA():
    def explained_variance(y_true, y_pred):
        return 1 - Rvar(y_true, y_pred)
    return explained_variance
...
def g_cod():
    def cod(y_true, y_pred):
        #return COD(y_true, y_pred)
        return (SSRES(y_true, y_pred)/SSTOT(y_true))
    return cod
...
def g_codINV():
    def cod(y_true, y_pred):
        return 1/COD(y_true, y_pred)
        #return (SSRES(y_true, y_pred)/SSTOT(y_true))
    return cod

def gdep_KrsEvar():
    def keras_explained_variance(y_true, y_pred):
        return 1/explained_variance_score(k.array(y_true), k.array(y_pred))
    return keras_explained_variance
...
def true_negatives(ytrue, ypredict, label=0):
    ...
    print()
    """
    quck = sum([ np.array_equal(yt, yp) for yt, yp in zip(ytrue, ypredict)])
    #print('correct total: {}'.format(quck))
    c_z = sum([np.equal(yt, yp) and yp[0] == 0 for yp, yt in zip(ypredict, ytrue)])
...
def SM_Logit(Training, Testing, verbose=False):
    ...
    print('The log likelyhoods are:')
    for l, lbl in zip(loglikly, X):
        print('{:s}: {:>10}'.format(lbl, l))
    print()
    #show_labeled_list(loglikly, X)
    print('pvalue for {:s}: {:f}\n'.format(X2.columns.values.tolist()[0], model2.pvalues.loc[X.columns.values.tolist()[0]]))
    y_pred = model2.predict(Xt2, linear=True)
    # print(y_pred)
    yp = list()
    predicted_prob = list()
