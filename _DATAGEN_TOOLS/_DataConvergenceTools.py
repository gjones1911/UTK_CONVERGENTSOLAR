...
class CONVERGENT_DATA_GEN:
    ...
    def __init__(self,
               ...
               usecols=(None, None), generate_extra_variables=True,
                **kwargs):

        self.destination=destination
        # load and merge the two data sets
        # add aditional solar metrics
        # deepsolar_df = pd.read_csv(deepsolarpath, usecols=usecols[0], low_memory=False, index_col=0)
        deepsolar_df = pd.read_excel(deepsolarpath, usecols=usecols[0],index_col=0)
        seeds_df = pd.read_csv(seedspath,   usecols=usecols[1],low_memory=False)
        dsss = [deepsolar_df, seeds_df]
        joins = ('fips', 'geoid')
        self.DataDict = dict()
    ...
    def generate_green_travelers(self, merged=None, green_travelers=None):
        ...
        self.DataDict[
            'Green_Travelers'] = "combination of rates of persons using a bicycle, walking, or working from home"
    ...
    def recode_locale_data(self):
        ...
        self.merged['URBAN'] = self.merged['locale_(urban)'].values + self.merged['locale_(town)'].values
    ...
    def add_binary_policies(self):
        ...
        self.merged.loc[
            [a + b == 2 for a, b in zip(self.merged[nm].tolist(), self.merged[pt].tolist())], 'Policy_Combo'] = 4
        self.merged.loc[
            [a + b == 2 for a, b in zip(self.merged[nm].tolist(), self.merged[litx].tolist())], 'Policy_Combo'] = 5
        self.merged.loc[
            [a + b == 2 for a, b in zip(self.merged[pt].tolist(), self.merged[litx].tolist())], 'Policy_Combo'] = 6
        self.merged.loc[[a + b + c == 3 for a, b, c in zip(self.merged[nm].tolist(), self.merged[pt].tolist(),
                                                           self.merged[litx].tolist())], 'Policy_Combo'] = 7
    ...
    def add_renewable_gen(self, year='2015'):
        ...
        add_renewable_gen_df_df(self.merged,
                                ...
                                cold='state')
        self.DataDict[
            'renew_prod'] = "Renewable (overall) energy producation based on data from:ttps://www.eia.gov/electricity/data/state/ "
        self.DataDict[
            'hydro_prod'] = "hydro renewable energy producation based on data from:ttps://www.eia.gov/electricity/data/state/ "
        self.DataDict[
            'solar_prod'] = "solar renewable  energy producation based on data from:ttps://www.eia.gov/electricity/data/state/ "
    ...
    def combine_edu(self):
        ...
        self.DataDict[
            'education_high_school_or_above_rate'] = " percentage of persons high graduate education level or above (DeepSolar)"
        ...
        self.DataDict[
            'education_high_school_or_below_rate'] = "percentage persons with high school education or less (DeepSolar)"
        ...
        self.DataDict[
            'education_bachelor_or_above_rate'] = "percentage persons with bachelor degress or above (DeepSolar)"
        self.merged['education_bachelor_or_above_rate'] = create_combo_var_sum(self.merged, bachelor_above)

        self.DataDict[
            'educated_population_rate'] = "population 25 or above with some college education (SEEDSII) / population (DeepSolar)"
    ...
    def add_home_age_range_combos(self):
        ...
        self.merged['Mid_Agedwellings'] = self.merged["hu_vintage_1960to1970"].values + self.merged[
    ...
    def add_hotspots(self, ):
        ...
        # add the hot spots
        self.DataDict[
            'Hot_Spots_hh'] = "indicates that the census tract is in the 95th percentile for per household adoption"
        add_HOTSPOTS(df=ds_df, var="number_of_solar_system_per_household", new_var_name='Hot_Spots_hh',
                     percentile=.949, verbose=True)

        self.DataDict[
            'Hot_Spots_hown'] = "indicates that the census tract is in the 95th percentile for per homeowner adoption"
        add_HOTSPOTS(df=ds_df, var='PV_HuOwn', new_var_name='Hot_Spots_hown',
                     percentile=.949, verbose=True)

        self.DataDict[
            'Hot_Spots_AvgAr'] = "indicates that the census tract is in the 95th percentile average panel area"
    ...
    def add_summed_seedsii_vars(self, ):
        ...
        self.merged[verylow_hh_r] = smart_df_divide(self.merged['verylow_own_hh'].values,
                                                    self.merged['total_own_hh'].values)
        ...
        self.DataDict[
            'verylow_own_Sbldg_rt'] = "% very low income owned homes suitable for solar installations (SEEDSII)"
    ...
    def add_regionalindicators(self, ):
        ...
        self.DataDict[
            'Top_3_States'] = "binary indicator of a census tract being in one of the top 3 adoptions per household census tracts which are CA, NV, AZ"
    ...
    def random_interactions(self):
        ...
        self.DataDict[
            'Savings_potential'] = "product of daily solar radiation (DeepSolar), total owned developable plane area (SEEDSII derived), dollars per kilo watt hour(DeepSolar)"
        self.merged['Savings_potential'] = self.merged['daily_solar_radiation'].values * self.merged[
            'total_own_devp'].values * self.merged['dlrs_kwh'].values
        self.DataDict[
            'Income_x_College_Edu'] = "prduct of median household income(DeepSolar) and education_college percent (DeepSolar)"
        self.merged['Income_x_College_Edu'] = self.merged["median_household_income"] * self.merged[
            'education_college_rate']
        self.DataDict[
            'Cost_x_Consumption'] = "dollars per kilo watt hour (DeepSolar) * average montlhy energy consumption (kwh)(DeepSolar)"
        ...
        self.DataDict[
            'suburban_diversity'] = "binary suburban locale * racial diversity(rate of race mixture) (DeepSolar) "
        self.merged['suburban_diversity'] = self.merged['locale_(suburban)'] * self.merged['diversity']
        self.DataDict['rural_diversity'] = "binary rural locale * racial diversity(rate of race mixture) (DeepSolar) "
        self.merged['rural_diversity'] = self.merged['locale_(rural)'] * self.merged['diversity']

    def split_solarresources(self):
        # add designations for the higher and lower than average daily solar areas
        self.DataDict[
            'High_Solar_Areas'] = "binary indicator for census tracts above the average value for daily solar radiation"
        self.merged['High_Solar_Areas'] = np.full(len(self.merged), 0.0)
        self.merged.loc[
            self.merged['daily_solar_radiation'] > self.merged['daily_solar_radiation'].mean(), 'High_Solar_Areas'] = 1
        self.DataDict[
            'Low_Solar_Areas'] = "binary indicator for census tracts below the average value for daily solar radiation"
        self.merged['Low_Solar_Areas'] = np.full(len(self.merged), 0.0)
        self.merged.loc[
            self.merged['daily_solar_radiation'] < self.merged['daily_solar_radiation'].mean(), 'Low_Solar_Areas'] = 1
        self.DataDict[
            'DS_HighSolar'] = "binary indicator for census tracts above the threshold found by DeepSolar in daily solar radiation where adoption seemed to spike"
